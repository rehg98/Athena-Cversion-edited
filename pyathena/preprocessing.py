...
def processing_zprof_dump(h,rates,params,zprof_ds,hstfile):
    ...
    scaleH = np.interp(zpw.taxis,h.index,h['H'])
    Hidx=np.less(np.abs(zpw.zaxis.data[:,np.newaxis]),scaleH[np.newaxis,:])
    ...
    for i,ph in enumerate(['_2p','_h','','_c','_u','_w','_h1','_h2',]):
        ...
        #density within H
        d_within_H=(zp.sel(fields='d')/(1.e-20+zp.sel(fields='A'))*Hidx).sum(dim='zaxis')/Hidx.sum(axis=0)
        h_zp['nmean{}'.format(ph)]=d_within_H
        ...
        h_zp['momflux_{}{}'.format('ubd',ph)] = \
          h_zp['flux_{}_M3{}'.format('ubd',ph)]*topflux
        h_zp['momflux_{}{}'.format('lbd',ph)] = \
          h_zp['flux_{}_M3{}'.format('lbd',ph)]*topflux
        h_zp['momflux_{}{}'.format('bd',ph)] = \
          h_zp['flux_{}_M3{}'.format('bd',ph)]*topflux
         
        h_zp['Ekflux_{}{}'.format('ubd',ph)] = \
          (h_zp['flux_{}_E1{}'.format('ubd',ph)] + \
           h_zp['flux_{}_E2{}'.format('ubd',ph)] + \
           h_zp['flux_{}_E3{}'.format('ubd',ph)])*toEflux
        h_zp['Ekflux_{}{}'.format('lbd',ph)] = \
          (h_zp['flux_{}_E1{}'.format('lbd',ph)] + \
           h_zp['flux_{}_E2{}'.format('lbd',ph)] + \
           h_zp['flux_{}_E3{}'.format('lbd',ph)])*toEflux
        h_zp['Ekflux_{}{}'.format('bd',ph)] = \
          h_zp['Ekflux_{}{}'.format('ubd',ph)] + \
          h_zp['Ekflux_{}{}'.format('lbd',ph)] 

        h_zp['Ethflux_{}{}'.format('ubd',ph)] = \
          h_zp['flux_{}_P{}'.format('ubd',ph)]*toEflux
        h_zp['Ethflux_{}{}'.format('lbd',ph)] = \
          h_zp['flux_{}_P{}'.format('lbd',ph)]*toEflux
        h_zp['Ethflux_{}{}'.format('bd',ph)] = \
          h_zp['Ethflux_{}{}'.format('ubd',ph)] + \
          h_zp['Ethflux_{}{}'.format('lbd',ph)] 

        for zheight in [500,1000,1500]:
          if zp.zaxis.max() > zheight:
            zp_upper=zp.sel(zaxis=zheight,method='nearest')
            zp_lower=zp.sel(zaxis=-zheight,method='nearest')
            zstr='{:02d}'.format(int(zheight/100))
            fluxes=['massflux','momflux','Ekflux','Ethflux']
            variables=['d','M3','Ekin','P']
            factors=[toflux,topflux,toEflux,toEflux]
            for flux_,var_,conv_ in zip(fluxes,variables,factors):
              fstr='{}_out'.format(flux_)
              if var_ is 'Ekin':
                pflx_=zp_upper.sel(fields='pFzE1')
                pflx_+=zp_upper.sel(fields='pFzE2')
                pflx_+=zp_upper.sel(fields='pFzE3')
                mflx_=zp_upper.sel(fields='mFzE1')
                mflx_+=zp_upper.sel(fields='mFzE2')
                mflx_+=zp_upper.sel(fields='mFzE3')
              else:
                pflx_=zp_upper.sel(fields='pFz{}'.format(var_))
                mflx_=zp_upper.sel(fields='mFz{}'.format(var_))
              h_zp['{}_u{}{}'.format(fstr,zstr,ph)] = (pflx_/area)*conv_
              h_zp['{}_l{}{}'.format(fstr,zstr,ph)] = (-mflx_/area)*conv_
              h_zp['{}_{}{}'.format(fstr,zstr,ph)] = \
                h_zp['{}_u{}{}'.format(fstr,zstr,ph)]+ \
                h_zp['{}_l{}{}'.format(fstr,zstr,ph)]
             
              fstr='{}_in'.format(flux_)
              h_zp['{}_u{}{}'.format(fstr,zstr,ph)] = (mflx_/area)*conv_
              h_zp['{}_l{}{}'.format(fstr,zstr,ph)] = (-pflx_/area)*conv_
              h_zp['{}_{}{}'.format(fstr,zstr,ph)] = \
                h_zp['{}_u{}{}'.format(fstr,zstr,ph)]+ \
                h_zp['{}_l{}{}'.format(fstr,zstr,ph)]
             
              h_zp['{}_{}{}'.format(flux_,zstr,ph)] = \
                h_zp['{}_out_{}{}'.format(flux_,zstr,ph)]+ \
                h_zp['{}_in_{}{}'.format(flux_,zstr,ph)]
            ...
            h_zp['{}_{}{}'.format(fstr,zstr,ph)] = \
              0.5*(h_zp['{}_u{}{}'.format(fstr,zstr,ph)]+ \
                   h_zp['{}_l{}{}'.format(fstr,zstr,ph)])
